...
- [ ] Add `MemoryRetentionAction` with `keep`, `compress`, `cool_down`, `archive`, `tombstone`, and `purge`.
- [ ] Add `MemoryRetentionPolicy` with bounded defaults:
...
- [ ] Add `MemoryRetentionDecision` containing `memory_id`, `action`, `score`, `reason`, `protected`.
- [ ] Test strict validation and sane defaults.
...
- [ ] Add async `delete(memory_id: UUID) -> bool`.
- [ ] Verify physical delete removes records but not audit history.
- [ ] Use this only for old tombstones/archives, never active records.
...
- [ ] Implement `MemoryMaintenanceService.evaluate(record, policy)`:
...
- [ ] Implement `maintain(apply=False)` dry-run by default, and `maintain(apply=True)` that applies safe actions.
- [ ] Add audit events for applied archive/tombstone/purge decisions.
...
- [ ] Group repeated or related normal memories by tenant/user/project/conversation/category.
- [ ] When a group has at least `compress_after_source_count` active normal memories, create or request a summary memory before archiving sources.
- [ ] Keep the summary source ids and archive original records with `archive_reason=consolidated_into:<summary_id>`.
- [ ] Never compress locked/core records without explicit user confirmation.
...
- [ ] Ensure Hot memories/experiences rank before Warm.
- [ ] Ensure Cold is retrieved only on strong relevance.
- [ ] Ensure Archive/Tombstone never inject into ordinary runtime context.
- [ ] Add per-source and total context budget tests.
...
- [ ] Apply `ConfidenceCalibrationService` to stale beliefs, experiences, strategies, and skill candidates.
- [ ] Keep conflicting records as `UNRESOLVED` unless evidence gap is sufficient.
- [ ] Ensure important inferred records remain candidates until verified by outcome or user confirmation.
...
- [ ] Generate a candidate experience from an interaction.
- [ ] Confirm or activate it.
- [ ] Run context routing and verify it is injected only when relevant.
- [ ] Record a failed outcome and verify confidence drops.
- [ ] Run maintenance and verify repeated raw memories are condensed or archived.
...
- [ ] Document memory lifecycle: Candidate → Active/Hot → Warm → Cold → Archive/Tombstone → Purge.
- [ ] Document deletion policy: compress first, archive second, purge only after retention windows or explicit deletion.
- [ ] Update local handoff with verification, risks, and remaining UI work.
