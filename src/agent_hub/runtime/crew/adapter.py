...
_CHARACTER_MODEL_SHEET_PROMPT_CONSTRAINT = (
    ...
    "生成逻辑必须按 Character Identity + Look / Costume + Pose + Scene + Shot Prompt 组合执行；"
    "Character Identity 负责这个人是谁，锁定脸型、五官比例、眼睛间距、鼻型、嘴型、下颌线、"
    "肤色、年龄特征、发际线、基础发型和身体比例；Look / Costume 只负责当前穿什么，"
    "只记录服装、鞋履、首饰、包、帽子、材质、颜色和穿着方式。"
    "不要把人物身份和服装绑定为同一个资产，换装必须从原始 Identity Reference 出发，"
    "不得以上一个场景图连续编辑导致累计漂移。"
    ...
    "图内尽量不要写文字，优先使用编号、图标、空白栏或简短符号分区；"
    "文字说明放在结构化产物元数据里，不依赖图片内文字传递关键信息。"
    "如确需图片内栏目名，只允许极短且容易生成正确的栏目标题，例如主图、三视图、表情、服装、道具、色卡；"
