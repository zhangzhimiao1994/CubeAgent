...
describe("operational management pages", () => {
  ...
  it("docks the latest running agent process above the composer while older process cards stay in history", async () => {
    ...
    const footer = view.container.querySelector(".chat-sticky-footer") as HTMLElement | null;
  ...
  it("keeps a clear mobile hierarchy for chat sessions, content, and the fixed composer", async () => {
    ...
    expect(screen.getByRole("navigation", { name: "会话导航" })).not.toBeNull();
    const stream = screen.getByRole("region", { name: "主对话内容" });
    const footer = document.querySelector(".chat-sticky-footer") as HTMLElement;
    const mobileHint = view.container.querySelector(".chat-mobile-module-strip") as HTMLElement;
    expect(stream).not.toBeNull();
    expect(footer).not.toBeNull();
    expect(mobileHint.textContent).toContain("conversation_id");
    expect(mobileHint.textContent).toContain("conv-");
    expect(mobileHint.textContent).not.toContain("当前模块");
    expect(mobileHint.textContent).not.toContain("自动");
    expect(screen.getByRole("button", { name: "跳到最新消息" })).not.toBeNull();
    expect(footer.parentElement).toBe(stream.parentElement);
    expect(stream.querySelector(".run-process-summary")).toBeNull();
