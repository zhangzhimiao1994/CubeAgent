...
它把 Web 控制台、飞书/通道入口、模型池、主 Agent 决策、工作流、Skill/MCP 治理、计划任务、多媒体生成、OpenClaw 电脑/服务器操作、统一记忆 / 经验中心和审计日志放在同一个系统里。

[English README](README.md)

## 能做什么

- 作为语音陪伴机器人的云端大脑：树莓派只做麦/喇叭与 WebSocket，回合走既有 RunService / 记忆注入 / 模型池。部署与整链路调试见 [docs/voice-companion-deploy-debug.md](docs/voice-companion-deploy-debug.md)；派上开箱见 `device/README.md`，架构见 `docs/architecture.md`。
...
历史会话行提供按原思路新建分支的入口；分支引用激活后，输入区只显示明确的取消引用控件，不再要求每条消息手动打开 Handoff。输入框固定在底部，对话内容在上方滚动；当前运行中的子 Agent 调度卡片会停靠在输入框上方，下一轮交互开始后，上一轮卡片回到对应历史对话位置。完整的项目级 Vibe Coding（生成项目、读写代码、运行测试、审查、调试并再次验证）留给后续 harness/runtime 改造：当前 UI 不暴露 Vibe Coding 按钮，后端 metadata 字段保留作未来接入。在 harness 接入前，代码类请求属于建议型能力：系统可以读取用户提供的代码上下文、说明问题、制定实现计划、生成补丁建议或文件产物，但不会直接修改用户源码树。正在运行的对话可以在输入区停止生成；对话里检测出的计划任务提案，也可以在创建持久记录前取消。

派单和讨论运行会显示紧凑的运行过程卡片，而不是直接铺开完整轨迹。运行中的卡片靠近固定输入区，已完成的卡片保留在对应历史轮次里。点击后进入子 Agent 工作席：每个 Agent 独立显示工作中、等待中、失败、已下班等状态。抽屉默认展示分类摘要和重点卡片，详细事件字段、工具 payload、模型元数据和产物元数据不会直接铺开；需要点开对应卡片并展开完整字段后查看。这样既能减少长流程的 UI 噪音，也保留排查和审计所需的信息。

长对话由对话框架处理。当历史内容接近主 Agent 模型上下文窗口时，系统会自动压缩旧轮次，保留初始目标、长期约束和最新结论，再把压缩上下文传入下一轮。

Hermes 学习不是原始聊天记录本身，也不再作为独立管理页面暴露。学习记录会进入统一记忆 / 经验中心内的学习台账，并带一条中文摘要；可复用 Cognitive 经验会作为候选经验出现，只有确认后才会影响后续运行。运行时注入按作用域过滤：`用户记忆` 只影响同一用户，`根记忆` 才能影响同租户内其他用户。
...
记忆 / 经验中心统一展示普通记忆、Hermes 学习记录和 Cognitive 经验候选。主界面使用摘要列表，避免把完整学习台账直接铺开；点击摘要卡片后在详情弹窗中查看完整内容、证据/冲突、置信度、使用次数和运行时注入状态，并在列表或详情中完成确认、拒绝、锁定/解锁或删除。旧 Hermes 路径只作为兼容入口跳转到统一中心。
...
- [语音陪伴：部署与整链路调试](docs/voice-companion-deploy-debug.md)
- [语音陪伴架构](docs/architecture.md)
- [机器人 `/api/robot/v1`](docs/cloud-robot-api.md)
- [树莓派前端](device/README.md)
